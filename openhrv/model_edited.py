...
class Model(QObject):
    ...
    hr_buffer_update = Signal(tuple)   # tuple(string, np.ndarray)
    ...
    def __init__(self):
        ...
        self._hr_buffer = np.full(HR_BUFFER_SIZE, -1, dtype=int)
        self._hr_seconds = np.arange(-HR_BUFFER_SIZE, 0, dtype=float)
    ...
    @Property(object)   
    def hr_buffer(self):
        return self._hr_buffer

    @Slot(object)
    def set_hr_buffer(self, value):
        self.hr_seconds = value
        self._hr_buffer = np.roll(self._hr_buffer, -1)
        self._hr_buffer[-1] = self.validate_hr(value)
        self.hr_buffer_update.emit(("HR", self.hr_buffer))
        self.compute_local_hrv()

    def validate_hr(self, value):
        if value > 220:
            print(f"Correcting invalid HR: {value}")
            return np.median(self._hr_buffer[-11:])
        return value
    ...
    @property
    def hr_seconds(self):
        return self._hr_seconds

    @hr_seconds.setter
    def hr_seconds(self, value):
        self._hr_seconds = self._hr_seconds - value 
        self._hr_seconds = np.roll(self._hr_seconds, -1)
        self._hr_seconds[-1] = -value 
